...
public class ChapterInfo implements Serializable {
    ...
    public void initChapterInfo(String chapterContentHtml,
        ...
        List<String> contentArr = StringUtil.splitStringByMaxCharList(chapterContentText, singleLineChars);
        this.setChapterContentList(contentArr);
    }

    /**
     * 初始化章节行数信息
     * @param prevReadLineNum 上一行阅读的行数
     * @param nextReadLineNum 下一行阅读的行数
     * @param lastReadLineNum 最后阅读的行数
     * @param singleLineChars 单行最大字符数
     */
    public void initLineNum(int prevReadLineNum, int nextReadLineNum, int lastReadLineNum, int singleLineChars) {
        ...
        if (StringUtils.isNotBlank(chapterContentStr)) {
            List<String> contentArr = StringUtil.splitStringByMaxCharList(chapterContentStr, singleLineChars);
            this.setChapterContentList(contentArr);
        } else {
