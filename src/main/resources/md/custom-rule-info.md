...
>> </li>
>> ```
>> 为基准，所以想要获取url信息，那么要填写：`.li-dd a`， 等同于js的`document.querySelectorAll('.book-list ul li')[0].querySelector('.li-dd a').href`。；  
>>
>> 提示：  
>> - 获取的url信息是标签的`href`属性的值，所以最终需要定位的元素必须是a标签；
>> - 此处可以通过在结尾添加`@back:`或`@front:`标记，以达到在获取的url信息前面或者后面添加额外的内容；
>>   - `@back:`代表在url后面添加内容，例如：`.li-dd a @back:index.html`，最终获取的url信息为：`/book/1/index.html`；
>>   - `@front:`代表在url前面添加内容，例如：`.li-dd a @front:info`，最终获取的url信息为：`info/book/1/`；
>>   - 这俩标签非必填，但若需要填写，则必须在末尾添加`@back:`或`@front:`标记，可以同时存在两个标记；
>> 
> 
> ____________________________
...
>>     - `bookImgUrl`：小说图片链接；
>>
>>   如果选择入参的参数类型是`BookInfo`对象，那么在`<package_import></package_import>`部分请填写：`import com.wei.wreader.model.BookInfo;`。  
>>   提示：
...
>> ```xml
>> <java>
>>   <package_import>import com.wei.wreader.util.data.*;</package_import>
>>   <code>
